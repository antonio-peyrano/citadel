    ...
    class entidades
        {
             ...
             * y funcionalidades de la interfaz del modulo de entidades.
             */
            
            //ATRIBUTOS APLICABLES AL MODULO catEntidadesphp
            private $Condicionales = "";
            private $Sufijo = "ent_";
            private $Entidad = "";
            private $idTEntidad = NULL;
            private $Consulta = "SELECT idEntidad, Entidad, Status FROM catEntidades WHERE Status=0";
            //FIN DE DECLARACION DE ATRIBUTOS APLICABLES AL MODULO catEntidadesphp
            
            //ATRIBUTOS APLICABLES AL MODULO opEntidadesphp
            private $idEntidad = NULL;
            private $cntView = 0;
            //FIN DE DECLARACION DE ATRIBUTOS APLICABLES AL MODULO opEntidadesphp
            ...
            //PROCEDIMIENTOS APLICABLES AL MODULO catEntidades.php
            public function getEntidad()
                {
                    /*
                     * Esta funcion retorna el valor del nombre de entidad.
                     */
                    return $this->Entidad;
                    }

            public function getidTEntidad()
                {
                    /*
                     * Esta funcion retorna el valor de idTEntidad.
                     */
                    return $this->idTEntidad;
                    }
            ...
            public function setCatParametros($Entidad, $idTEntidad)
                {
                    /*
            ...
            public function evaluaCondicion()
                {
                    ...
                    if(!empty($this->getEntidad()))
                        {
                            $this->Condicionales = ' AND Entidad LIKE \'%'.$this->getEntidad().'%\'';
                            }
                            
                    if(!empty($this->getidTEntidad()))
                        {
                            if($this->getidTEntidad()!="-1")
                                {
                                    $this->Condicionales= $this->Condicionales.' AND idTEntidad = \''.$this->getidTEntidad().'\'';
                                    }
                            }
                                                        
                    return $this->Condicionales;                            
                    }                    
            //FIN DE DECLARACION DE PROCEDIMIENTOS APLICABLES AL MODULO catEntidades.php
            
            //PROCEDIMIENTOS APLICABLES AL MODULO opEntidades.php.
            public function getRegistro($idRegistro)
                {
                    /*
                     * Esta funcion obtiene el dataset del registro de entidades apartir del ID proporcionado.
                     */
                    global $username, $password, $servername, $dbname;
                    
                    $objConexion= new mySQL_conexion($username, $password, $servername, $dbname); //Se crea el objeto de la clase a instanciar.
                    $Consulta= 'SELECT idEntidad, Entidad, idTEntidad, Status FROM catEntidades WHERE idEntidad='.$idRegistro; //Se establece el modelo de consulta de datos.
                    $dsEntidad = $objConexion -> conectar($Consulta); //Se ejecuta la consulta.
                    
                    $RegEntidad = @mysqli_fetch_array($dsEntidad,MYSQLI_ASSOC);//Llamada a la funcion de carga de registro de usuario.
                    return $RegEntidad;
                    }                    
            ...
            //FIN DE DECLARACION DE PROCEDIMIENTOS APLICABLES AL MODULO opEntidades.php            
            }
?>